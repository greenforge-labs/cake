...
@dataclass
class Subscribers:
    input: cake.Subscriber[Bool] = field(default_factory=cake.Subscriber[Bool])
...
@dataclass
class NoNodeSectionContext(cake.Context):
...
T = TypeVar("T", bound=NoNodeSectionContext)
...
def run(context_type: type[T], init_func: Callable[[T], None]):
    ...
    node = rclpy.create_node("no_node_section")
    ...
    ctx.subscribers.input._initialise(ctx, Bool, "/input", QoSProfile(history=HistoryPolicy.KEEP_LAST, depth=5, reliability=ReliabilityPolicy.BEST_EFFORT))
